...
function activate(context) {
    ...
    wss.on("connection", (socket) => {
      ...
      vscode.workspace.onDidChangeTextDocument((event) => {
        ...
        if (vscode.workspace.workspaceFolders) {
          ...
          const allFiles = [];
          let allFileModifiedContents = ""; // Initialize container for all file data
          ...
          for (const filePath of allFiles) {
            ...
            if (filePath === fileUri) {
              ...
              allFileModifiedContents += fileData; // Append to the container
              // Send file content as plain text
            }
          }
          socket.send(allFileModifiedContents);
        }
      });
    });
  ...
  function sendAllFiles(socket) {
    if (vscode.workspace.workspaceFolders) {
      ...
      let allFileContents = ""; // Initialize container for all file data
      ...
      for (const filePath of allFiles) {
        ...
        allFileContents += fileData; // Append to the container
      }

      // Send the entire combined file data in one message
      socket.send(allFileContents);
    }
  }
}
